 ...
 */
 ...
 /*
  * in Versions later than 1.2.5pre1, fd files created with a 1.3 (UTF-8)
  * version may be read in but no attempt is made to translate non-ASCII
  * characters. While reading such a newer fd file, a warning will occur.
  */
...
load_fd_header( void )
{
    ...
    /* preset: -1 ; not read in, alos do not write out, else write the received */
    ...
    /* Warn if the fd file has been generated by a (newer) utf8 version */
    ...
    if ( fd_magic > MAGIC6 )
    {
        ...
        if ( ! fdopt.conv_only )
            fl_show_alert_f( 0, "Warning:\fFile %s\n"
                             "was created with a newer fdesign version using "
                             "UTF-8,\n"
                             "the new output file may not be compatible.",
                             tmp );
        else
            M_warn( "", "Warning: File %s was created with a newer fdesign "
                    "version using UTF-8,, the new output file may not be "
                    "compatible", tmp );
...
save_forms( const char *str )
{
    ...
    /* For UTF-8 compatiblity of fd files: if read != -1 then also write */

    if ( fdopt.use_x11_fonts > -1 )
        fprintf( fp, "Use-X11-Fonts: %d\n", fdopt.use_x11_fonts );
